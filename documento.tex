...
\begin{frame}
	...
	\begin{itemize}
	  ...
	  e métodos:
	  \begin{itemize}
	    \item Público -- Atributos e métodos podem ser acessados de qualquer lugar; 
	    \item Protegido -- Usar um único sublinhado (\_numero) indica que o membro deve ser acessado apenas internamente ou por subclasses. É uma convenção de aviso, não um bloqueio técnico; ou
	    \item Privado -- Usar dois sublinhados (\_\_saldo) aciona o name mangling (alteração de nome), tornando o atributo difícil de acessar fora da classe.
	  \end{itemize}
	\end{itemize}
\end{frame}
